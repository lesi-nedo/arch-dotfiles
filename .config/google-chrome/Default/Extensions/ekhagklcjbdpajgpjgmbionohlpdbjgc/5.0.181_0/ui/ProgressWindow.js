...
Zotero.UI.ProgressWindow = class ProgressWindow extends React.PureComponent {
  ...
  handleAlerts(items) {
    ...
    if (savingMultipleItems) {
      ...
      alertQueue = [{
        text: Zotero.getString("progressWindow_savingItems", [this.announceAlerts, topLevelTitles]),
        id: 'saving_items_count'
      }];
    } else {
      alertQueue = [{
        text: Zotero.getString("progressWindow_savingItem", shortenTitle(toplevelItems[0].title)),
        id: 'saving_items_count'
      }];
    }
    ...
    this.alertTimeout = setTimeout(() => {
      ...
      if (this.done) {
        alertQueue.push({
          text: Zotero.getString("progressWindow_allDone"),
          id: "allDone"
        });
      }
      for (let {
    ...
    this.setState(prevState => {
      return {
        targets: [...prevState.targets.map(target => {
          return ids.has(target.id) ? Object.assign({}, target, {
            expanded: true
          }) : target;
        })]
      };
  ...
  renderHeadline() {
    ...
    }, this.state.headlineText, this.state.targets ? this.renderHeadlineSelect() : this.state.target ? this.renderHeadlineTarget() : "", React.createElement("div", {
      id: "messageAlert",
      role: "alert",
      style: {
        fontSize: 0
  ...
  renderHeadlineSelect() {
    ...
    return React.createElement(React.Fragment, null, React.createElement("select", {
      ...
      return React.createElement("option", props, row.name);
    })), React.createElement("button", {
      className: "ProgressWindow-disclosure" + (this.state.targetSelectorShown ? " is-open" : ""),
  ...
  renderHeadlineTarget() {
    return React.createElement(React.Fragment, null, React.createElement(TargetIcon, {
      type: getTargetType(this.state.target.id)
  ...
  onTagAutocompleteShown(extraHeight) {
    this.setState({
      extraHeightForTagAutocomplete: extraHeight
    });
  }
  renderTargetSelector() {
    if (!this.state.targetSelectorShown) return "";
    const filterElement = React.createElement("div", {
      className: "ProgressWindow-filterWrapper"
    }, React.createElement("input", {
      className: "ProgressWindow-filterInput",
      onInput: this.setFilter,
      placeholder: this.text.filterPlaceholder
    }), React.createElement("button", {
      className: "ProgressWindow-cross hidden",
      onClick: this.clearFilter,
      tabindex: -1
    }));
    const targetSelectorElement = React.createElement("div", {
    ...
    if (this.canUserAddNote) {
      noteEditorElement = React.createElement("div", {
        className: "ProgressWindow-noteEditorRow"
    ...
    const tagsInputElement = React.createElement(TagsInput, {
    ...
    return React.createElement("div", null, filterElement, targetSelectorElement, noteEditorElement, tagsInputElement);
  }
  ...
  renderItem(item) {
    ...
    var iconStyle = Object.assign({}, {}, item.failed && {
    ...
    return React.createElement("div", {
      key: item.id,
      className: "ProgressWindow-item",
      style: itemStyle
    }, React.createElement("div", {
      className: "ProgressWindow-itemIcon",
      style: iconStyle
    }), React.createElement("div", {
  ...
  renderError(row, index) {
    ...
    if (err === "translationError") {
      ...
      contents = React.createElement("span", null, "An error occurred while saving this item. Try again, and if the issue persists see ", React.createElement("a", {
        href: url,
        title: url
      }, "Getting Help"), " for more information.");
    }
    return React.createElement("div", {
      className: "ProgressWindow-error",
      key: index,
      role: "alert"
    }, contents);
  }
  render() {
    ...
    }, this.renderHeadline(), this.renderTargetSelector(), this.renderProgress(), this.renderErrors());
  }
};
  ...
  render() {
    return React.createElement(Tree, {
      ...
      renderItem: (item, depth, isFocused, arrow, isExpanded) => {
        ...
        }, arrow), React.createElement(TargetIcon, {
          type: getTargetType(item.id)
        }), React.createElement("span", {
...
_defineProperty(TargetTree, "propTypes", {
...
class TagsInput extends React.Component {
  constructor(props) {
    ...
    _defineProperty(this, "removeTag", tag => {
      let selectedTags = new Set(this.props.selectedTags);
    ...
    _defineProperty(this, "onTagsInputChange", event => {
    ...
    _defineProperty(this, "onTagAutocompleteMouseDown", index => {
      this.isClickingTag = true;
      this.setState({
        currentTagIndex: index
      });
    });
    _defineProperty(this, "onTagAutocompleteMouseUp", index => {
    ...
    _defineProperty(this, "onTagsInputKeyDown", event => {
      let tags = this.getAvailableTags();
      if (event.key === "Enter") {
    ...
    _defineProperty(this, "onSelectedTagsKeyDown", event => {
    ...
    _defineProperty(this, "onTagsInputFocus", () => {
    ...
    _defineProperty(this, "onTagsInputBlur", () => {
  ...
  componentDidUpdate(_, prevState) {
    ...
    if (this.autocompleteRefs.length) {
      ...
      autocompleteRefNode?.scrollIntoView({
        block: 'nearest'
      });
    }
  ...
  render() {
    ...
    return React.createElement("div", {
      className: `ProgressWindow-targetSelectorTagsRow ${willShowAutocomplete ? 'with-autocomplete' : ''} ${this.props.selectedTags.size ? 'hasTags' : ''}`
    }, React.createElement("div", {
    ...
    }, Array.from(this.props.selectedTags).map((tag, index) => React.createElement("div", {
      key: tag,
      id: `tag_${index}`,
      ...
      className: "ProgressWindow-button",
      onClick: this.props.handleDone
    }, this.text.done), willShowAutocomplete ? React.createElement("div", {
      id: "tags-autocomplete",
      className: "ProgressWindow-autocomplete",
      role: "listbox",
      ref: this.autocompletePopupRef
    }, tags.map((tag, index) => React.createElement("div", {
      key: tag,
